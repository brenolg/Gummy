...
export const schema = yup.object({
  cardNumber: yup
    ...
    .test('cartao-valido', 'Número do cartão inválido.', (value) => {
  ...
  expiry: yup
    ...
    .test('validade-futura', 'Este cartão já está vencido.', (value) => {
      if (!value) return false
  ...
  cvv: yup.string().required('Informe o CVV.').matches(cvvRegex, 'CVV inválido.'),

  holderName: yup
    .string()
    .required('Digite o nome como está no cartão.')
    .matches(/^[A-Za-zÀ-ÿ\s]+$/, 'Use apenas letras.')
    .test('nome-composto', 'Digite o nome completo.', (value) => {
      if (!value) return false
...
export const schemaPix = yup.object({
  cpf: yup
    .string()
    .required('Informe seu CPF.')
    .test('cpf-valido', 'CPF inválido.', (value) => {
